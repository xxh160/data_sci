...
- 组长：潘昱光，学号191250109；
- 组员：袁军平，学号： 191250189， 侯为栋，学号191250045；
- 开源项目地址：https://github.com/xxh160/data_sci。

## 引言

这次大作业目的是分析从疫情开始到逐渐稳定这一时间段内大众心态的变化，并根据变化情况对未来做一定的预测。  
为了实现这一目标，我们需要做以下几个部分  
- 爬虫：获取数据
- 数据预处理：数据筛选
- 心态分析：建立心态词典
- 心态预测：曲线拟合心态走向，根据走向预测大众心态变化
...
原本的思路是按照不同的新闻或不同的视频之间评论数的差异，根据评论数的均值和方差，选出评论数较多，亦即较受关注的新闻/视频，但由于获取B站数据时，单一视频弹幕量巨大，且弹幕时间前后相差时间会很长，因而最终的数据并非按照视频/新闻为单位存储，而是按照时间，按天存储。  

而按天存储得到的数据会相对均匀，一段时间内起伏不会太大，即sigma不会非常大，且按天存储的数据不会像新闻评论数一样能直接反某条新闻的受关注程度，因而最终在预处理时，选择了筛选出在一段时间内评论数量较多的那些天做处理，具体方法是选择评论量大于mu-0.5*sigma的那些天，这样可以选择出在某一段时间内人们对疫情关心程度不是很低的时间，亦即能代表这一段时间大众心理的那些天
...
心态分析的基本思路是，b站爬取的原始疫情相关弹幕数据集经过预处理后，作为实验数据集。对实验数据集的近一百个视频 200万条弹幕进行TF-IDF高频词计算，从中构建心态词典，进而对这段时间内大众的心态进行分析。
...
对于疫情中大众心态变化的计算，我们最初希望通过分阶段来观察大众心态变化,但考虑到阶段内的天数过少，可能会造成较大误差等等原因，我们最终选择了在1月23日至6月末的的时间尺度上，观察大众在疫情下四种主要心态的变化。

我们选择了以下方法：

十四天为一个单位量,将该时间段内的弹幕分词,计算四类心情词语出现的频率，将心情出现频率作为当天的对应情绪值，具体代码为anaylys.py里的analys()

考虑到各种心态强度的不同，为了更直观地分析结果，我们从以下两个角度分析结果
...
我们去除了弹幕高频词里的百度停词表中的停词,对所有弹幕生成了一个词云，以及对疫情最严重的时期的一月以及二月这个阶段生成了一个词云
...
![b站词云](img/b站分析结果/b站词云.png)

所有弹幕词云结果图：

![23月词云图](img/b站分析结果/23月词云图.png)
...
**文本情绪预测具体实现位于pyhanlp_predication/han.py   hanclass中的predict函数，具体实现可看源码**

训练好的模型读取数据预处理模块中得到的筛选数据，进行情感的预测。由于微博和B站的文本数据量差距较大，故分开来处理。每一天为一个分析单元，读取当天所有的评论数据计算其中`pos`评论的比例来大致估算当天的民众整体情绪状况。结果通过`matplotlib`类库得到折线图，`weibo`的分析图如下所示。
...
总的来说，这次项目我们分为了数据获取，数据预处理，心态分析以及情绪预测四部分。心态分析帮助我们更好地了解疫情过程中大众的心态变化,准确并客观地了解公众的网络社会心态与基于此呈现出的行为规律。情绪预测部分实现了对大众情绪可能的预测以及研究,可以帮助更好地去形成一个良好积极的社会心态。除此以外，我们通过一些可视化技术将研究结果直观地展示出来。总之，我们希望这个研究可以产生一些积极的对社会有用的成果。所有代码可以见我们的github仓库。https://github.com/xxh160/data_sci
